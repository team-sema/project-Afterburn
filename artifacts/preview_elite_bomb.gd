...
func _run() -> void:
	...
	for column in names.size():
		...
		if raster.load_svg_from_string(source, 4.0) != OK:
			...
			return
		if column == 3:
			raster.save_png("res://artifacts/enemy_elite_bomb_preview.png")
	...
	var result := root.get_texture().get_image().save_png("res://artifacts/elite_bomb_design.png")
	print("elite bomb design preview: ", "PASS" if result == OK else "FAIL")
