...
AI 산출물은 저장소 규칙·시스템 스펙에 부합하는지 검수한 뒤 반영했으며, 재미와 밸런스적 요소는 사람이 직접 플레이하며 확정했습니다.
...
|---|---|---|
| 1. 기능 정의 | 관련 스펙과 기존 기능을 찾아 영향 범위를 확인한다. | 기능의 목적·우선순위·플레이 방식·제약을 결정한다. |
| 2. 작업 준비 | `/feature` 입력을 받으면 최신 `main`을 확인하고 브랜치를 만든다. 작업 범위를 알리고 카드를 `doing`으로 옮긴 뒤 스펙과 작업 목록을 작성한다. | `/feature`를 입력하고 AI가 제시한 작업 범위와 예상 동작을 확인한다. |
| 3. 구현 | 합의된 스펙과 작업 목록에 따라 코드와 씬을 수정한다. `docs/spec/`을 갱신하고 테스트와 자체 검사를 수행한다. | 중간 빌드를 플레이하고 조작감·연출·밸런스에 관해 피드백한다. |
| 4. 반영 승인 | 변경된 파일, 검사 결과와 남은 확인 사항을 정리한다. | 결과를 확인하고 저장소에 반영해도 될 때 `/push`를 입력한다. |
| 5. 저장소 통합 | `/push` 입력을 받으면 변경 내역, 스펙, 작업 목록과 완료 조건을 대조한다. 검사에 통과하면 카드를 `review`로 옮기고 커밋한 뒤 `main`에 머지하고 원격 저장소에 반영한다. | 충돌이나 작업 범위 변경이 발견되면 처리 방향을 결정한다. |
