...
| 2 | 적 증강 풀 수 **3** 표기 (실제 6) | `docs/design/overview.md` 스펙 트래킹 표 |
| 3 | 카드·인덱스 열 불일치 2건 | `docs/board/cards.json`, `docs/design/history/README.md` |
...
| 6 | 폴더 맵에 `pickups/`·`weapon_test/`·`tests/`·루트 조립 파일 누락 | `docs/design/overview.md`, `godot-development.mdc` |
| 7 | 구조 부채(씬 인라인 풀·그룹 조회·trait 중복 등)가 어디에도 기록되지 않음 | `docs/design/gaps.md` |

## 동작 (AC)

1. **오그먼트 트리거 정본화** — 규칙·README가 `docs/design/augments.md`와 같은 설명(XP 충족 후 `C`, 적 60초 주기)을 쓴다. 점수는 표시·기록용임을 명시한다.
