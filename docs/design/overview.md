...
이 기획서는 게임의 의도·확정 규칙·구현 참고를 함께 관리한다. 문서 선택과 운영 기준은 [기획서 입구](README.md)를 따른다.

## 핵심 루프

1. 적을 처치해 점수·XP 획득
2. XP 충족 후 **C**로 플레이어 오그먼트 선택 · 시퀀스 ELITE 관문의 엘리트 처치 후 Threat 상승 및 적 오그먼트 선택 (일시정지)
...
루트에 `gameplay.tscn`·오퍼/진행 컨트롤러·레지스트리가 함께 있다. 하위 폴더로 옮기는 정리는 백로그 (`docs/design/gaps.md`).

## 주제별 기획 문서

기획서 브라우저(`docs/design/index.html` · `design.js`)에서 아래 주제를 읽는다. 과거 설계와 변경 내역은 Git 이력에서 확인한다.
...
| 일반 적 | `enemies/` | 일반 유닛별 HP·사격·행동 |
| 엘리트 | `elites/` | 독립 유닛별 전투 규칙·구현/시안 상태 |
