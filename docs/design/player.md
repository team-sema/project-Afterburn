...
여러 무기를 동시에 운용하고 무기 모듈·시설 선택으로 한 판의 빌드를 만든다. 피격 코어와 실드로 회피 결과를 읽을 수 있게 한다.
...
- HUD: `WeaponLoadoutHud` — 48px 장착 베이와 28px 장착 모듈 flat-top 헥스를 변이 맞닿는 벌집으로 배치한다. 템플릿(`%BaySlotTemplate` · `%ModuleHexTemplate` · `%SelectedWeaponHex`) 복제. 호버/클릭 포커스 → 하단 `선택된 무기` | `장착된 모듈` + 설명(전투 수치는 인게임 비표시). 설명은 고정 2줄·말줄임이며 우측 레일의 최소 크기를 늘리지 않는다. 증강 오퍼 중 신규 무기는 첫 빈 베이에 점멸 미리보기하며 만석이면 생략하고, 무기 모듈은 대상 베이와 장착 모듈 영역의 기존/첫 빈 칸에서 다음 레벨 아이콘을 점멸한다
- 증강 리롤: `AugmentOfferController.max_reroll_count`(임시 기본 2) · 런당 `remaining_reroll_count` · 현재 포커스 카드 한 장만 교체
...
- 좌우 STATUS 사이의 중앙 플레이필드 안에 선택창 전체를 넣고 3장의 세로형 카드를 실린더 단면처럼 겹쳐 표시한다. 포커스 카드는 전면·불투명, 좌우 카드는 후면·축소·반투명 상태이며 좌우 입력으로 순환한다. 선택 중 별도 조작 설명 문구는 표시하지 않는다
- 전장과 좌우 HUD는 계속 보이고, 적용 대상 미리보기는 오퍼 내부 복제 패널 대신 기존 우측 STATUS를 사용한다
- `STAT_MULTIPLIER`는 플레이어 오퍼 풀에 없다. `FACILITY_EFFECT` 카드 포커스·호버는 우측 STATUS에서 같은 primary module tag의 장착 슬롯 외곽선을 강조하고 첫 빈 범용 슬롯에 후보 시설 아이콘을 점멸한다. 만석이면 점멸을 생략한다. 무기 Kind 카드는 시설 tag 강조를 강제하지 않는다
- 신규 무기 획득은 빈 베이가 있으면 우측 STATUS 첫 빈 베이에 후보 아이콘을 점멸한다. 만석이면 점멸을 생략하고 확정 후 교체 창을 연다. 무기 모듈 카드는 대상 병기 베이를 강조한다
- `범용 슬롯 +1`은 무기 Kind 포커스를 포함해 플레이어 오퍼에 항상 표시한다(`스킵 후 범용 슬롯 확장 n → n+1`). 포커스·호버하면 우측 STATUS의 다음 육각 슬롯 한 칸이 일시정지 중에도 점멸한다. 선택하면 용량을 1개 늘리고 오퍼를 종료한다. 최대 15칸이면 `스킵 (범용 슬롯 MAX 도달)`로 비활성 표시하고 건너뛴다
- 좌우 방향키는 카드 포커스를 순환한다. 길게 누르면 키가 눌린 동안에만 최소 0.45초 간격으로 회전하고, 쿨다운 중 입력과 키를 뗀 뒤의 예약 회전은 버린다. 위·아래 방향키와 Tab은 `카드 → [R] 리롤 → 범용 슬롯 +1` 순서로 이동하며 `ui_accept`는 현재 포커스된 카드 또는 버튼을 실행한다
- `[R] 리롤 (n)` 버튼은 현재 포커스 카드 한 장만 교체하며 다른 카드와 포커스는 유지한다
...
- 시작 블래스터, 빈 베이 획득, 만석 교체가 아래 장착 규칙대로 동작한다.
- 무기 교체 후 이전 무기의 모듈이 남지 않고 재획득 시 초기 상태로 시작한다.
- 시설 효과·실드 충전·포커스 미리보기가 장착 결과와 일치한다.

검증 참고: `tests/weapon_replace_delete_reroll_test.gd`. Godot 실행은 `tools/run-godot.cmd`를 사용한다.
