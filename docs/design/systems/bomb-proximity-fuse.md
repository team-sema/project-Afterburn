...
2. `BombProximityFuseComponent`: 플레이어가 `trigger_radius`(72) 안이면 정지 → `arm_duration`(2s) 동안 적색 점멸 `flash_count`(3)회 → 자폭
3. 폭발 VFX scale · AOE 반경 = `base_explosion_radius(40) * blast_size_multiplier(1.5)`
...
- 기본 `DestroyedComponent` VFX는 자폭 시 끊고 컴포넌트가 확대 폭발을 스폰한다
