...
대조 기준: 2026-09-15 · 게임 코드(`feature/encounter-sequence` 포함) + 본 문서 트리.
...
| 오그먼트 풀 가중치 | **현행 설계** — `offer_weight` × 범주 배율. 정본 [오그먼트](augments.md) |
...
- Threat/Encounter 로스터·시퀀스 확장 시 [run-pacing](run-pacing.md)과 동기화 (병렬 시퀀스 브랜치 병합 때 문서 갱신)
