...
XP를 모은 뒤 플레이어가 선택 시점을 정하며, 무기·모듈·시설의 선택으로 빌드를 바꾼다. 적 증강은 다음 구간의 압력을 높이는 선택이다.
...
적용: `ShipFacilityApplier` · `ShipCombatBuffController` · `EngineBoostComponent` · `HurtComponent`(iframe). 과거 결정 이력: [`docs/design/history/facility-weapon-modules.md`](history/facility-weapon-modules.md).
...
- XP 조건 충족만으로 오퍼가 자동으로 열리지 않으며 C 입력으로 열린다.
- 풀 스캔·대상 무기·최대 단계 등 아래 자격 조건을 만족한 카드만 등장한다.
- 리롤은 포커스 카드 한 장만 바꾸고 런당 잔여 횟수를 정확히 차감한다.

검증 참고: `tests/augment_pool_data_driven_smoke_test.gd`. Godot 실행은 `tools/run-godot.cmd`를 사용한다.
