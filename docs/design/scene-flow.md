...
개발 시험의 공통 입구는 `labs/lab_hub.tscn`이다. 탄막·패턴, 무기·증강, 증강 카드 UI를 목록에서 방향키/Enter로 선택한다. 허브에서 연 Lab은 F1로 목록에 복귀하며, 씬은 하나씩 실행한다. 기존 개별 씬의 F6 실행도 유지한다. 본 게임 시작 씬은 변경하지 않는다. 탄막 스크립트 작업 규칙은 [전투](combat.md)를 따른다.
