...
`threat_monitor/threat_monitor_lab.tscn`을 에디터에서 열고 **F6**으로 실행한다. 화면 중앙은 본 게임과 같은 240×360 플레이 영역이며, 왼쪽에는 현재값·완화값·최근 최고값·누적량과 실시간 그래프, 오른쪽에는 다섯 구성 값·동시 공격원·공격 종류·표본 개수가 표시된다. 함선은 계측 도중 파괴되지 않으며, **R** 또는 다시 시작 버튼으로 장면을 초기화할 수 있다.
