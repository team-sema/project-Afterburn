...
- 신규 탄의 예측은 최대 50ms, 파동 한 주기당 최소 24구간으로 표본화하고 남은 수명과 화면 이탈에서 끝낸다. 궤적 구간 수와 무관하게 밀도는 한 발로 센다. 정지한 신규 탄도 공간 점유에 포함한다. 캡슐은 외접 원을 쓰므로 정지 안전 비율은 보수적 근사이며 실제 피격 판정이 아니다.
...
- ACTION_RATE는 패턴 시간 배율에 적용하며 탄 이동/Behavior는 바꾸지 않는다. 초기 지연은 기존처럼 실제 게임 시간이다.
...
2026-09-17 API 검토에서 표현력 공백을 세 갈래로 나눴다. (a) `_init()`의 GDScript 루프로 단계를 펼치면 이미 가능한 것(부분 반복·다중 속도 링·버스트별 절대각)은 API 추가 없이 [작성 관용구](../barrage-api.md#작성-관용구)로 문서화한다. (b) 런타임에만 정해지는 것은 아래 원시 연산으로 추가한다. (c) 구조 결정이 필요한 것은 파라미터화를 구현하고 탄→탄 발사는 자리만 확정한다. 발사 시점 난수(jitter)는 이번 범위에서 제외했다. `await` 기반 코루틴 패턴은 사전 검증·저장·시간 분할 결정론을 잃으므로 채택하지 않는다.
...
- 공통 Bullet Lab은 안전 비율 토글(기본 ON)과 FPS/평균 프레임 간격(ms)을 제공한다. OFF는 Lab의 SafeSpaceMeter 자동 샘플링을 중단하고 정지 안전 비율 대신 OFF를 표시한다. 다시 ON하면 측정을 초기화하고 정상 샘플 간격으로 재개한다. 선택은 패턴 변경·다시 발사·pause 후에도 유지한다. FPS는 실제 경과 시간 기준 0.5초 구간의 화면 프레임 수로 계산하며 pause 중에도 표시한다. 토글 직후에는 FPS 구간도 초기화하여 이전 모드와 섞지 않는다.
...
- 외부 효과 후속 구현 시 완료 조건: 효과 중첩/만료·pause·탄별 격리, 개입 직전/직후 위치 연속성, 과거 꼬리 불변, 몸체/판정/경로 예측 동일 궤적 확인. 외부 효과 적용 API는 아직 노출하지 않는다.
...
- 성능 보정: 궤적 조회에서 불필요한 속성 해석을 생략하고 단일 방향 파동은 직접 평가한다. 안전 비율 격자는 선분의 확장 경계 안에 있는 칸만 검사하되 기존 중심점 거리 판정을 유지한다. 동일 예제의 몸체·배치·안전 비율 CPU 시간을 수정 전후 비교한다.
...
- 정지 안전 비율 = 1 − 위험 칸 수 / 전체 칸 수. 플레이어 위치는 사용하지 않는다.
- 신규 탄의 예측 반경은 Behavior 전체의 최대 판정 배율을 쓰므로 실제보다 보수적이다.
...
- Bullet Lab은 정지 안전 비율을 표시하며, 감시 범위 밖의 적과 적탄은 계산에서 제외한다.
- 적탄 경로·접촉 피해 몸체·Bomb 접근 영역이 정지 안전 비율에 반영되고, 성긴 탄막은 높은 비율을 유지한다.

검증 참고: `tests/bullet_cancel_reward_smoke_test.gd` · `tests/safe_space_meter_test.gd`. Godot 실행은 `tools/run-godot.cmd`를 사용한다.
